...
Paddle::Paddle(bn::fixed radius, bn::fixed angle) : _radius(radius),
                                                    _angle(angle),
...
Projectile Paddle::shoot()
{
    bn::fixed shootSpeed = 3; // TODO: put elsewhere
    auto [sin, cos] = bn::degrees_sin_and_cos(_angle);
    return Projectile(_sprite.position(), 
                     {-cos * shootSpeed, -sin * shootSpeed},
                      bn::sprite_items::bullet);
}